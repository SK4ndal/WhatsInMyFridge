...
The core inventory foundation story introduced the first backend implementation for WhatsInMyFridge. The implemented backend needs to expose inventory and foodstuff HTTP APIs, persist reusable foodstuff configuration separately from owned inventory items, and support later stories such as expiry awareness, waste tracking, shopping-list intelligence, meal recommendations, and meal planning.
...
- SQLAlchemy models are the source for persisted inventory and foodstuff boundaries.
