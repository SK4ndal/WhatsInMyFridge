...
if [ -L "$README_LINK" ]; then
  CURRENT_TARGET="$(readlink -f "$README_LINK")"
  EXPECTED_TARGET="$(readlink -f "$README_TARGET")"
  if [ "$CURRENT_TARGET" != "$EXPECTED_TARGET" ]; then
    echo "Refusing to replace existing work/README.md symlink: points to $(readlink "$README_LINK")"
    exit 1
  fi
